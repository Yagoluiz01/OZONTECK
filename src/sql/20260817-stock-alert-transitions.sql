-- OZONTECK - Transições atômicas de estoque para alertas administrativos
-- Data: 2026-08-17
--
-- Esta migration é retrocompatível: preserva os campos atuais das RPCs e
-- acrescenta apenas `stock_changes` ao JSON retornado. A API usa esse campo
-- para disparar alertas de estoque baixo/esgotado sem inferir valores fora
-- da transação e sem alterar a regra de reserva de 24 horas.
...
BEGIN
  ...
  LOOP
    ...
    v_current_stock := v_previous_stock - v_quantity;
    ...
    v_stock_changes := v_stock_changes || jsonb_build_array(
      jsonb_build_object(
...
BEGIN
  ...
  LOOP
    UPDATE public.products
       SET stock_quantity = COALESCE(stock_quantity, 0) - v_item.quantity,
           updated_at = now()
     WHERE id = v_item.product_id
     RETURNING stock_quantity, name, sku
          INTO v_current_stock, v_product_name, v_product_sku;
    ...
    v_stock_changes := v_stock_changes || jsonb_build_array(
      jsonb_build_object(
        ...
        'previous_stock', v_current_stock + v_item.quantity,
        'current_stock', v_current_stock
      )
    );
  END LOOP;
  ...
  RETURN jsonb_build_object(
    'success', true,
    'reserved', true,
    'reason', 'reserved_again',
    'stock_changes', v_stock_changes
  );
END;
$$;

-- Mantém as RPCs restritas à chave service_role, igual à migration principal.
