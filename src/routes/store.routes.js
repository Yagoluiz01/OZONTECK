...
router.post("/payments/pix", async (req, res) => {
  try {
    const orderNumber = String(
      req.body?.orderNumber || req.body?.order_number || ""
    ).trim();
    ...
    // A recuperação só pode operar sobre o pedido que pertence ao token
    // gerado pelo exchange seguro.
    if (!isValidOrderAccessToken(req, order)) {
      return res.status(403).json({
        success: false,
    ...
    const currentOrderStatus = String(order.order_status || "")
      .trim()
      .toLowerCase();
    const currentGateway = String(order.payment_gateway || "")
      .trim()
      .toLowerCase();
    const currentRawStatus = String(order.payment_raw_status || "")
      .trim()
      .toLowerCase();
    ...
    if (
      ["shipped", "enviado", "delivered", "entregue"].includes(
        currentOrderStatus
      )
    ) {
      return res.status(409).json({
    ...
    // Se existe PIX ainda válido, reutiliza o mesmo QR Code.
    // Evita dois pagamentos simultâneos para o mesmo pedido.
    if (
      currentGateway === "mercado_pago_pix" &&
      /^\d{1,32}$/.test(currentPaymentId)
    ) {
      let existingPayment;
      ...
      try {
        ...
        console.error(
          "ERRO AO VERIFICAR PIX ATUAL ANTES DA RECUPERAÇÃO:",
        ...
        return res.status(502).json({
          ...
          message:
            "Não foi possível verificar o Pix atual. Tente novamente em instantes.",
        });
      }
      ...
      const existingReference = String(
        existingPayment?.external_reference || ""
      ).trim();
      const metadataOrderId = String(
        existingPayment?.metadata?.order_id || ""
      ).trim();
      const currencyId = String(existingPayment?.currency_id || "BRL")
        .trim()
        .toUpperCase();
      ...
      if (
        ...
        return res.status(409).json({
          ...
          message:
            "O Pix atual não corresponde aos dados oficiais deste pedido.",
        });
      }
      ...
      const expirationMs = expirationText
        ? Date.parse(expirationText)
        : NaN;
      ...
      const canReuseExisting =
        ["pending", "in_process"].includes(existingStatus) &&
        !isExpiredByDate;
      ...
      if (!canReplaceExisting) {
        return res.status(409).json({
          ...
          message:
            "O pagamento atual ainda está sendo processado. Aguarde a confirmação antes de gerar outro Pix.",
        });
      }
    } else if (
      currentGateway === "mercado_pago_card" &&
      currentPaymentId
    ) {
      ...
      if (!canSwitchFromCard) {
        return res.status(409).json({
          ...
          code: "PAYMENT_STILL_PROCESSING",
          message:
            "O pagamento com cartão ainda está sendo processado. Aguarde a confirmação antes de trocar para Pix.",
        });
      }
    }
    ...
    if (!reservation?.reserved) {
      return res.status(409).json({
        ...
        message:
          "Um dos produtos ficou sem estoque antes da nova tentativa de pagamento.",
      });
    }
    ...
    // Dados do pagador vêm do pedido oficial no servidor.
    // CPF não volta ao navegador e não é necessário para PIX quando
    // o gateway aceita nome/e-mail.
    const customer = {
      ...
      if (reservation?.reason === "reserved_again") {
        try {
          await releaseOrderStock(
            order.id,
            "pix_retry_generation_failed"
          );
        } catch (releaseError) {
          console.error(
            "ERRO AO DEVOLVER ESTOQUE APÓS FALHA NO NOVO PIX:",
    ...
    if (!paymentUpdate.ok) {
      return res.status(500).json({
        success: false,
        message:
          "O novo Pix foi criado, mas não foi possível salvar a referência no pedido.",
      });
    }
    ...
    return res.status(Number(error?.statusCode) || 500).json({
      success: false,
      message:
        error.message || "Não foi possível gerar um novo Pix.",
    });
  }
});
  ...
  try {
    ...
    return res.status(200).json({
      ...
      order: {
        ...
        shipping_delivery_time: Number(order.shipping_delivery_time || 0) || null,
      },
    });
  } catch (error) {
